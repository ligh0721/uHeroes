...
public class CameraFollowPlayer : MonoBehaviour
{
	...
	void FixedUpdate()
    {
        if (follow && GamePlayerController.localClient && GamePlayerController.localClient.unitCtrl)
        {
            var player = GamePlayerController.localClient.unitCtrl.gameObject.transform;
            ...
            float deltaXLeft = player.position.x - pos.x + slowX;  // < 0
            if (deltaXLeft < 0)
            {
            ...
            {
                float deltaXRight = player.position.x - pos.x - slowX;  // > 0
                if (deltaXRight > 0)
                {
            ...
            float deltaYLeft = player.position.y - pos.y + slowY;  // < 0
            if (deltaYLeft < 0)
            {
            ...
            {
                float deltaYRight = player.position.y - pos.y - slowY;  // > 0
                if (deltaYRight > 0)
                {
